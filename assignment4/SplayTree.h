// Assignment 4
// SplayTree.h: Header file for SplayTree class
#ifndef SPLAY_TREE_H
#define SPLAY_TREE_H
...
class SplayTree
{
  public:
    ...
    // Initiate recursive in-order display from the root node
    void tdisplay(ostream& out = std::cout) const {
        tdisplay(root, 0, out);
    }

    // Return the total count of even elements in the tree
    int counteven() const {
...
private:
   ...
   void printTree( BinaryNode *t ) const
    ...
    // Recursively perform in-order traversal and print each node with depth-based indentation
    void tdisplay(BinaryNode* t, int d, ostream& out) const {
        if (t != t->left) {
            tdisplay(t->left, d+1, out);
            out << std::string(d, '>') << t->element << endl;
            tdisplay(t->right, d+1, out);
        }
    }

    // Recursively count even-valued nodes in the subtree rooted at t
    int counteven(BinaryNode* t) const {
        if (t == t->left) return 0;
        return counteven(t->left) + counteven(t->right) + !(t->element & 1);
    }
